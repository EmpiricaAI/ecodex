...
Alpha. **`main` tracks upstream codex 0.149** (synced 2026-08-25 from the stable `rust-v0.149.0` tag; latest release [v0.149.0](https://github.com/EmpiricaAI/ecodex/releases), 2026-08-25). The version tracks the upstream [openai/codex](https://github.com/openai/codex) base this build is derived from — hence the jump from `0.2.x` at the first rebased release (`0.146.0`, which shipped on the pinned `0.146.0-alpha.10.1` base while upstream was alpha-only above `0.137`; upstream has since resumed stable tags). Tracking the base keeps the client version ecodex reports compatible with OpenAI's per-model version gates (so frontier models like gpt-5.6 work over ChatGPT-subscription auth); ecodex patches increment as `0.149.x`, then move to the new base on each upstream re-sync. (First public release v0.1.0 was 2026-06-02.) The release pipeline (gated build/test/clippy, GH release, crates.io publish for owned crates, Homebrew tap) lives in `scripts/release.sh`.
...
- **Framework skills** (`pinned: true`): `epistemic-transaction`, `empirica-constitution`, and `epistemic-persistence-protocol` are flagged as session-wide framework skills rather than per-task tools. Since 0.149 (upstream removed its skill-injection mechanism) their bodies are no longer auto-reinjected each session; the skills catalog prompt instead directs the model to proactively read framework `SKILL.md` files and re-read them after `/compact`, and anything needing guaranteed ambience routes through codex's native AGENTS.md instructions channel — recovering ~12.5K tokens of per-session frame overhead that previously priced out small-context models.
