...
pub struct PluginManifestPaths<Resource> {
    ...
    /// ecodex T74: optional executable the host invokes on a render tick to
    /// contribute a status line (resolved from the manifest's `statusline`
    /// field). `None` when the plugin did not declare one.
    pub statusline: Option<Resource>,
    /// ecodex Tx-AI/3: cross-cwd writable roots the plugin's runtime needs
    /// (resolved from the manifest's `writableRoots` field). Each entry is a
    /// fully-resolved locator (tilde already expanded at manifest load time).
    /// Empty when the plugin did not declare any.
    pub writable_roots: Vec<Resource>,
}
    ...
    pub fn try_map_resources<Mapped, Error>(
        ...
        Ok(PluginManifest {
            ...
            paths: PluginManifestPaths {
                ...
                hooks,
                statusline: statusline.map(&mut map).transpose()?,
