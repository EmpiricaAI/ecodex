...
pub mod legacy_core {
    ...
    // ecodex: the TUI plugin-statusline refresh (T41) constructs a local
    // PluginsManager; expose core's canonical composition helper so the TUI
    // does not need a direct codex-core dependency.
    pub mod plugins {
        pub use codex_core::plugins_manager_for_config;
    }
}
