...
fn resolve_manifest_runtime_paths(
    ...
    for raw in paths {
        ...
        // Upstream migrated plugin paths to PathUri; keep the same
        // absolute/traversal validation, then convert for the manifest field.
        match AbsolutePathBuf::try_from(expanded)
            .map_err(|err| err.to_string())
            .and_then(|abs| {
                PathUri::from_host_native_path(abs.as_path()).map_err(|err| err.to_string())
            }) {
