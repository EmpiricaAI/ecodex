...
struct ThreadSettingsBuildParams {
    ...
    model: Option<String>,
    // ecodex extension (T78): provider override carried alongside `model`
    // so a curated-model picker entry that routes to a different provider
    // reaches core and hot-swaps the ModelClient.
...
impl TurnRequestProcessor {
        ...
        let thread_settings = self
            .build_thread_settings_overrides(
                ...
                ThreadSettingsBuildParams {
                    ...
                    model: params.model,
                    // ecodex extension (T78): forward the picker's staged
                    // provider override so core hot-swaps the ModelClient.
    ...
    async fn build_thread_settings_overrides(
        ...
        let has_any_overrides = has_environment_override
            ...
            || permissions.is_some()
            || model.is_some()
            // ecodex extension (T78): a provider-only swap (model unchanged,
            // provider differs) must still register as an override so it
            // reaches core and hot-swaps the ModelClient.
        ...
        let thread_settings = self
            .build_thread_settings_overrides(
                ...
                ThreadSettingsBuildParams {
                    ...
                    permissions: params.permissions,
                    model: params.model,
                    // ecodex extension (T78): provider override on the
                    // standalone thread/settings/update path.
                    model_provider: params.model_provider,
                    service_tier: params.service_tier,
                    effort: params.effort,
                    summary: params.summary,
