...
enum McpSessionApprovalKey {
    // ecodex: server-wide session approval stays scoped to the providing plugin,
    // so an approval granted under one plugin never covers a same-named server
    // exposed by another (upstream keys approvals by plugin_id since #44655).
    Server {
        server: String,
        plugin_id: Option<String>,
    },
...
fn session_mcp_tool_approval_key(
    ...
    match scope {
        McpSessionApprovalScope::Server if key.connector_id.is_none() => {
            Some(McpSessionApprovalKey::Server {
                server: key.server,
