...
async fn run_remote_compact_task_inner_impl(
    ...
    let compaction_output_result = if let Some(client_session) = client_session {
        ...
        // ecodex T78: services.model_client is ArcSwap<ModelClient>;
        // .load() returns a Guard that derefs to &ModelClient.
        let mut owned_client_session = sess.services.model_client.load().new_session();
        run_remote_compaction_request_v2(
            sess,
            turn_context,
