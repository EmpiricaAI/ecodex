...
pub(crate) async fn run_turn(
    ...
    // ecodex T78: services.model_client is ArcSwap<ModelClient> (hot-swappable
    // mid-session via /model). `.load()` returns a Guard that derefs to
    // &ModelClient transparently for the .new_session() call.
    let mut client_session = prewarmed_client_session
        .unwrap_or_else(|| sess.services.model_client.load().new_session());
    ...
    loop {
        ...
        match sampling_request_result {
            Ok((sampling_request_output, sampling_request_input)) => {
                ...
                if !needs_follow_up {
                    last_agent_message = sampling_request_last_agent_message;
                    // ecodex addition (goal f0004294): permission mode used by
                    // the maintained TaskCompleted event below.
                    ...
                    // Adopt upstream's restructured AfterAgent dispatch
                    // (supersedes our former inline HookEvent::AfterAgent loop).
