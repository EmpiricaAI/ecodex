...
async fn thread_settings_update(
    ...
    SessionSettingsUpdate {
        ...
        // ecodex extension (T78): forward the staged provider override so
        // apply()/update_settings hot-swaps the ModelClient (ArcSwap).
        model_provider,
        multi_agent_mode,
...
pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
    // ecodex addition (goal f0004294): fire SessionEnd before the runtime
    // teardown so plugin handlers capture final session state while
    ...
    crate::hook_runtime::run_session_end_hooks(sess, &session_end_ctx, pre_shutdown_turn_count).await;
