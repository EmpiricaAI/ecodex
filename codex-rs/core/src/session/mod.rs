...
impl Session {
    ...
    pub(crate) async fn update_settings(
        ...
        let (
            ...
            let mut state = self.state.lock().await;
            let previous_provider_name = state.session_configuration.provider.info().name.clone();
            ...
            let provider_swap = if updated.provider.info().name != previous_provider_name {
                Some(ProviderSwapPlan {
                    new_provider: updated.provider.info().clone(),
                    new_provider_name: updated.provider.info().name.clone(),
                    config: Arc::clone(&updated.original_config_do_not_use),
                    session_source: updated.session_source.clone(),
                    originator: updated.originator.clone(),
                })
            } else {
                None
            };
    ...
    pub(crate) async fn swap_model_client_to_provider(&self, plan: ProviderSwapPlan) {
        ...
        let new_client = crate::client::ModelClient::new(
            ...
            plan.config
                .features
                .enabled(codex_features::Feature::ConcurrentReasoningSummaries),
            self.services.attestation_provider.clone(),
            plan.config.http_client_factory(),
        );
    ...
    async fn record_conversation_items_impl(
        ...
        let items = items
            ...
            .collect();
        self.record_prepared_conversation_items_impl(
            turn_context,
            items,
            image_preparations,
            emit_display,
    ...
    async fn record_prepared_conversation_items_impl(
        &self,
        turn_context: &TurnContext,
        items: Vec<ResponseItemEnvelope>,
        image_preparations: Vec<ImagePreparationMetadata>,
        ...
        self.persist_rollout_items(&rollout_items).await;
        // ecodex: `record_conversation_items_silent` (hook-injected
        // additionalContext) records into history/rollout but never renders
        // to the client — model reads it, screen stays clean.
        if emit_display {
    ...
    pub(crate) async fn build_initial_context_with_world_state(
        ...
        // ecodex sync note: convert the multi-agent fragment to a Send `ResponseItem`
        // at capture time. ecodex's pinned-skills injection below introduces an
        // `.await` that upstream lacks; holding the non-Send `Box<dyn
        // ContextualUserFragment>` across it would make this future non-Send
        // (SessionTask::run requires Send). Converting eagerly keeps only Send
        // values alive past the await.
        let mut initial_multi_agent_mode: Option<ResponseItem> = None;
        for fragment in world_state.render_full() {
            match fragment.role() {
                ...
                "developer" if fragment.markers().0 == MULTI_AGENT_MODE_OPEN_TAG => {
                    initial_multi_agent_mode = Some(fragment.into_boxed_response_item());
                }
                "developer"
        ...
        if let Some(initial_multi_agent_mode) = initial_multi_agent_mode {
            items.push(initial_multi_agent_mode);
        }
        if let Some(contextual_user_message) =
            crate::context_manager::updates::build_contextual_user_message(contextual_user_sections)
        {
        ...
        // ecodex note (0.149.0 sync): the pinned-skill BODY auto-injection that
        // lived here was dropped with upstream's removal of the SkillInjections
        // mechanism (David-ratified, decision artifact in empirica). The
        // `pinned` frontmatter flag survives on SkillMetadata; guaranteed
        // ambient content routes through the plugin's AGENTS.md channel instead.
    ...
    /// ecodex (Monitor / ntfy doorbell): deliver synthetic wake items
    /// (`<task-notification>` messages) to the agent.
    ///
    /// Upstream's mailbox rewrite removed `InputQueue::inject_response_items`;
    /// the Monitor/ntfy wake-delivery spine is remapped onto the mailbox API
    /// here. Each input item is wrapped as an [`InterAgentCommunication`] with
    /// `trigger_turn = true` and enqueued via
    /// [`InputQueue::enqueue_mailbox_communication`], then
    /// `maybe_start_turn_for_pending_work` either attaches the mailbox items to
    /// the active turn or wakes an idle session — preserving the original
    /// "inject now, else start a turn" semantics in one step.
    ///
    /// The mailbox always accepts, so this always returns `Ok(())`; the
    /// `Result` shape is retained so existing call sites (which fall back to
    /// `queue_response_items_for_next_turn` on `Err`) keep compiling.
    pub async fn inject_response_items(
        self: &Arc<Self>,
        input: Vec<ResponseInputItem>,
    ) -> Result<(), Vec<ResponseInputItem>> {
        for item in &input {
            let communication = Self::wake_communication_from_response_item(item, true);
            self.input_queue
                .enqueue_mailbox_communication(communication, /*parent_turn_id*/ None, /*root_turn_id*/ None)
                .await;
        }
    ...
    /// ecodex (Monitor): queue wake items to be delivered on the *next* turn.
    ///
    /// Remapped onto the mailbox API: items are enqueued with
    /// `trigger_turn = false` (so they do not wake an idle session), and the
    /// active turn (if any) is marked to defer mailbox delivery to the next
    /// turn via [`InputQueue::defer_mailbox_delivery_to_next_turn`]. This
    /// preserves the original semantics where items that could not attach to
    /// the current turn ride along on the following one.
    pub(crate) async fn queue_response_items_for_next_turn(&self, items: Vec<ResponseInputItem>) {
        for item in &items {
            let communication = Self::wake_communication_from_response_item(item, false);
            self.input_queue
                .enqueue_mailbox_communication(communication, /*parent_turn_id*/ None, /*root_turn_id*/ None)
                .await;
        }
        if let Some(sub_id) = self.active_turn_sub_id().await {
            self.input_queue
    ...
    /// Build a synthetic [`InterAgentCommunication`] carrying the plaintext of a
    /// Monitor/ntfy wake item. The author/recipient are the synthetic root
    /// agent path; non-text input variants are rendered via their debug form so
    /// nothing is silently dropped.
    fn wake_communication_from_response_item(
    ...
    /// Returns the `sub_id` of the currently active turn, if one is running.
    async fn active_turn_sub_id(&self) -> Option<String> {
        let active = self.active_turn.lock().await;
