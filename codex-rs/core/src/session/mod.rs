...
impl Session {
    ...
    pub(crate) async fn swap_model_client_to_provider(&self, plan: ProviderSwapPlan) {
        ...
        let new_client = crate::client::ModelClient::new(
            ...
            plan.config
                .features
                .enabled(codex_features::Feature::ContentItemKinds),
            plan.config
                .features
            ...
            // Same routing context the session was built with (cached per thread).
            self.services
                .thread_extension_data
                .get_or_init(|| plan.config.workspace_routing_context())
                .as_ref()
                .clone(),
        );
    ...
    pub(crate) async fn record_conversation_items(
        ...
        self.record_conversation_items_impl(
            turn_context,
            model_info,
            items,
            /*emit_display*/ true,
    ...
    pub(crate) async fn build_initial_context_with_world_state(
        ...
        // ecodex note: the pinned-skill BODY auto-injection that lived here was
        // dropped with upstream's removal of the SkillInjections mechanism
        // (David-ratified, decision artifact in empirica). The `pinned` flag
        // survives on SkillMetadata; guaranteed-ambient content routes through
        // codex's native AGENTS.md channel instead.
