...
async fn t78_swap_model_client_to_provider_invalidates_startup_prewarm() {
    ...
    let plan = crate::session::ProviderSwapPlan {
        new_provider_name: provider.info().name.clone(),
        new_provider: provider.info().clone(),
        config,
        session_source,
        originator: "test_originator".to_string(),
    };
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let services = SessionServices {
        ...
        // ecodex (T78): ModelClient behind ArcSwap for provider hot-swap.
...
async fn build_initial_context_reinjects_pinned_skill_bodies() {
    ...
    turn_context
        .extension_data
        .insert(codex_skills_extension::HostSkillsSnapshot::new(Arc::new(
            outcome,
        )));
