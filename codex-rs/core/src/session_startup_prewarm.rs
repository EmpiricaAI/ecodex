...
impl Session {
    pub(crate) async fn schedule_startup_prewarm(self: &Arc<Self>, base_instructions: String) {
        ...
        // ecodex (T78): model_client is an ArcSwap — deref via .load().
        if !self
            .services
            .model_client
            .load()
            .responses_websocket_enabled()
        {
            ...
            // ecodex T78: model_client is an ArcSwap; .load() derefs the current ModelClient.
            let model_client = self.services.model_client.load_full();
            tokio::spawn(async move {
...
async fn schedule_startup_prewarm_inner(
    ...
    let responses_metadata = session
        ...
        .await;
    // ecodex (T78): model_client is an ArcSwap, so go through `.load()`.
