...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    let status = {
        #[cfg(windows)]
        {
            ...
            // ecodex has no Windows standalone update action (see
            // tui/src/update_action.rs — Windows isn't distributed yet), so the
            // upstream PowerShell special-case for StandaloneWindows is dropped.
            let path_env =
