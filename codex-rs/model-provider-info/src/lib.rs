...
impl ModelProviderInfo {
    ...
    pub fn validate_bedrock_override(&self) -> Result<(), String> {
        let unsupported_fields = Self {
            ...
            // derived `Default` of `false`, so a bedrock entry deserialized from a config
            // that omits it would never equal `Self::default()`. It is not a
            // bedrock-tunable field, so compare it at the derived default to avoid a
            // false reject.
            supports_openai_builtin_tools: Self::default().supports_openai_builtin_tools,
            ..self.clone()
        };
