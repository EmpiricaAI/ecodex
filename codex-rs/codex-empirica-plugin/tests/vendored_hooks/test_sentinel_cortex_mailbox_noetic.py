...
Mirrors empirica PR #248 (tests/test_sentinel_cortex_mailbox_noetic.py) against
ecodex's VENDORED copy under assets/hooks_scripts/hooks/sentinel-gate.py. cortex
mailbox READS (inbox_poll/outbox_poll/get_proposal/archive_proposal) + the ack
(complete_proposal) must be classified noetic (present in NOETIC_MCP_CORTEX), and
a bare `mcp__cortex` namespace (codex/ecodex aggregation) must normalize to the
full op name so the same classification applies — "option 2". This is the case
that gated a mesh-woken practitioner's cortex_inbox_poll under codex.
"""
...
def test_praxic_cortex_tools_stay_out_of_noetic():
    # Real state mutations / ECO-gated — must NOT be noetic.
