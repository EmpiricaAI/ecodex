...
fn host_only_prompts_preserve_existing_behavior_with_and_without_aliases() {
    ...
    - Skill lifecycle (framework vs progressive — ecodex extension):
      - Skills with `pinned: true` in their SKILL.md frontmatter are FRAMEWORK skills: standing policy for the whole session, not per-task tools. Their bodies are NOT auto-injected — proactively Read their SKILL.md early (before the first related action) and re-Read after a `/compact`. Guaranteed-ambient framework content ships via AGENTS.md instead.
      ...
      - Re-Read trigger: if you decide to invoke any skill and its body is not visibly in recent context, open SKILL.md before acting. The list above tells you the skill exists and how to find it; the body is what tells you how to follow it correctly.
    "###);
...
fn mixed_catalog_prefers_executor_inclusion_over_total_aliased_inclusion() {
    ...
    let cloud_resource = "skill://cloud/one";
    let cloud_catalog = SkillCatalog {
        entries: vec![
            SkillCatalogEntry::new(
                SkillPackageId("c1".to_string()),
                SkillAuthority::new(SkillSourceKind::Cloud, "codex_apps"),
                "c1",
                "",
                SkillResourceId::new(cloud_resource),
            )
            .with_display_path(cloud_resource),
        ],
        warnings: Vec::new(),
    };
    ...
    let host = host.expect("host catalog should render");
    let executor = executor.expect("executor catalog should render");
    let cloud = cloud.expect("cloud catalog should render");
    ...
    assert_eq!(cloud.report.total_count, 1);
