...
pub enum HookEventName {
    ...
    // The 2 below mirror a wider hook surface so empirica's scripts can
    // wire to ecodex's hooks.json. Note: SessionEnd (plus PreCompact/
    // PostCompact/SubagentStart/SubagentStop) converged with upstream, so
    // we adopt upstream's variants above rather than maintaining our own.
