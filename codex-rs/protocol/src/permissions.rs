...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write_git(
        ...
        if writable_git {
            entries.push(FileSystemSandboxEntry::new(
                FileSystemPath::Special {
                    value: FileSystemSpecialPath::project_roots(Some(String::from(".git"))),
                },
                FileSystemAccessMode::Write,
            ));
        }
...
mod tests {
    ...
    fn writable_git_extends_to_linked_worktree_git_state() {
        ...
        for path in [
            ...
            assert!(
                on.can_write_local_path_with_cwd(&path, &worktree),
        ...
        for path in [
            ...
            assert!(
                on.can_read_local_path_with_cwd(&path, &worktree),
    ...
    fn writable_git_off_keeps_linked_worktree_git_state_protected() {
        ...
        for path in [
            ...
            assert!(
                !off.can_write_local_path_with_cwd(&path, &worktree),
