...
pub struct TurnStartParams {
    ...
    /// ecodex extension (T78): override the `model_provider` ID for this
    /// turn and subsequent turns. When set, the session re-resolves the
    /// provider from `config.model_providers[<id>]` and hot-swaps the
    /// session-shared ModelClient. Pair with `model` for atomic
